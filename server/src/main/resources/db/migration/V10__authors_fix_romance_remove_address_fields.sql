-- V10: Fix authors, remove Romance, add address name/phone
-- ─────────────────────────────────────────────────────────────────────────────

-- ── 1. Authors fix ────────────────────────────────────────────────────────────
-- V9 inserted authors with ON CONFLICT (name) DO NOTHING.
-- If V9 ran on a DB that already had partial data, some authors may be missing.
-- Re-insert any missing authors using INSERT ... ON CONFLICT DO NOTHING.
...
INSERT INTO authors (id, name, bio, genre, famous_works) VALUES
  ...
  ('e1000000-0000-0000-0000-000000000019',
   'Nicholas Sparks',
   'Nicholas Sparks is an American novelist and screenwriter. He has had several of his books turned into popular films, including The Notebook and A Walk to Remember. His novels are known for their romantic themes.',
   'Romance',
   'The Notebook, A Walk to Remember, Message in a Bottle, Dear John')
ON CONFLICT (id) DO NOTHING;

-- Re-link authors by name (safe to re-run — idempotent)
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Robert C. Martin')
  WHERE author = 'Robert C. Martin' AND author_id IS NULL;
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Martin Kleppmann')
  WHERE author = 'Martin Kleppmann' AND author_id IS NULL;
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Joshua Bloch')
  WHERE author = 'Joshua Bloch' AND author_id IS NULL;
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'James Clear')
  WHERE author = 'James Clear' AND author_id IS NULL;
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Frank Herbert')
  WHERE author = 'Frank Herbert' AND author_id IS NULL;
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Jane Austen')
  WHERE author = 'Jane Austen' AND author_id IS NULL;
...
UPDATE books SET author_id = (SELECT id FROM authors WHERE name = 'Paulo Coelho')
  WHERE author = 'Paulo Coelho' AND author_id IS NULL;
...
-- ── 2. Remove Romance — safe approach ────────────────────────────────────────
-- Move former Romance books to Fiction category (they are classic literary works).
-- This preserves foreign-key integrity for any existing cart_items or order_items
-- that reference these books. We do NOT delete any books.
-- Pride and Prejudice, The Notebook, Me Before You, The Fault in Our Stars,
-- It Ends with Us — all moved to Fiction.
UPDATE books
  SET category_id = 'b1000000-0000-0000-0000-000000000007'   -- Fiction
  WHERE category_id = 'b1000000-0000-0000-0000-000000000006'; -- Romance

-- Remove the Romance category row (safe now that no books reference it).
-- If any FK still references it (e.g. some other race condition), this is
-- protected by ON DELETE SET NULL on books.category_id.
DELETE FROM categories WHERE id = 'b1000000-0000-0000-0000-000000000006';
